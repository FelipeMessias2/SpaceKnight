extends Area2D
## Faixa de perigo por contato (espinhos ou ácido). O visual e a colisão são
## construídos em código a partir da largura, então uma única cena serve
## para faixas de qualquer tamanho, no chão ou no teto (rotation = PI).
