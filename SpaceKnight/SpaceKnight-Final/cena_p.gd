...
const CARTOES := {
	...
	2: ["ESTAÇÃO MERIDIAN",
		"Após um tempo lutando contra o cinturão, a nave não aguentou e caiu com tudo, mal sabia Kael que o terror estava apenas começando. A queda deixou nosso herói num laboratório soterrado.\nNos corredores, experimentos de gravidade ainda zumbem.\nEm algum lugar acima, uma nave de fuga espera."],
...
func _mostrar_cartao(numero: int) -> void:
	...
	# a tecla que fechou o diálogo da fase anterior ainda está pressionada nesta
	# frame; sem esperar ela soltar, o cartão se fecharia sozinho
	while Input.is_action_pressed("Avancar"):
...
func _carrega_fase(numero: int) -> void:
	...
	_forcar_camera_da_fase(fase_atual)
...
func _forcar_camera_da_fase(fase: Node) -> void:
	var p := fase.get_node_or_null("Player")
	if p != null and p.has_method("ativar_camera"):
		p.ativar_camera()
